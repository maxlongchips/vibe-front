...
两者的底层差异在于**上下文获取方式**：
- **Cursor** 用 RAG 向量搜索 — 快、省 token，但"代码语义相似 ≠ 代码上下文相关"
- **Claude Code** 用 grep/find/git — 贵、慢，但像开发者一样精准定位多文件关联
...
**Windows 用户注意：** Claude Code 需要 WSL2。在原生 cmd/PowerShell 中无法运行。
...
2. **指令越多，所有指令的质量都下降** — 不只是新增的那些。前沿模型线性退化，小模型指数退化。
3. **LLM 偏好上下文两端的指令** — 开头和结尾的指令被执行得更好。
4. **300 行以内，越短越好** — HumanLayer 自己的 CLAUDE.md 不到 60 行。

来自腾讯云文章《Claude Code 最佳实践的 8 条黄金法则》（作者是 7 年 Amazon/Disney CTO）的补充观点：

5. **CLAUDE.md 是"AI 的大脑，不是文档"** — 它应该读起来像"给未来的失忆版自己的核心笔记"，而不是 onboarding 文档。
6. **解释"为什么"，不只是"是什么"** — Claude 理解了动机，才能在边界情况下做出正确判断。
7. **持续更新** — 过时的 CLAUDE.md 比没有更糟。
...
还支持 `@import` 语法和 `.claude/rules/` 目录做路径级别的规则作用域。
...
```
Level 1: 元数据（始终加载）
  └─ YAML frontmatter: name + description（description 最重要，Claude 靠它决定何时调用）
...
5. **不要塞代码风格规则** — 用 linter 和 formatter；skill 应该提供领域专业知识。
...
来自腾讯云《Claude Code 省钱指南》的核心建议：用 Opus 做规划和架构，切换到 Sonnet 做实现。用 `/cost` 随时跟踪花费。
...
双击 `Shift+Tab` 进入 Plan Mode — Claude 会先分析和规划，再执行。来自社区经验：Plan Mode 的结果比直接提示好得多，因为它强制 Claude 先思考再行动。
...
# 基础设施自动化
claude -p "检查这个 Terraform 配置是否有安全问题" < main.tf
```

这才是企业级 AI 的真正价值 — 不只是写代码，而是自动化整个工程流程。
...
两种传输方式：
- **stdio** — 本地进程通信（Claude Code 默认）
- **SSE / streamable-http** — 网络通信（远程服务器，团队共享）
...
**Q: Token 消耗太快？**
A: 用 Sonnet 做日常编码，Opus 只用于架构决策。用 `/compact` 定期压缩上下文。用 `ccusage` 监控实时成本。
